...
if [ "$1""x" == "buildx" ]; then
    ...
    cp -v ../circle_scripts/deps_arm64_only.sh ./__temp__deps.sh
    rsync -avz --exclude=".localrun" --exclude="local.properties" ../ ./__temp__src/
    docker build -f Dockerfile_arm64_only -t trifa_maven_a2_001 .
    rm -Rf ./__temp__src/
...
rsync -avz --exclude=".localrun" --exclude="local.properties" ./circle_scripts/trifa_arm64_only.sh /root/work/circle_scripts/trifa_arm64_only.sh
...
## done in Dockerfile now ## bash ../circle_scripts/deps_arm64_only.sh || exit 1

cd /root/work/build_dir/
bash ../circle_scripts/trifa_arm64_only.sh || exit 1
...
system_to_build_for="trifa_maven_a2_001"
